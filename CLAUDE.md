...
- 담당 구역: 홈의 '담당 구역' 칸에서 **시·도**(서울·경기·부산·대구·대전·인천·울산·세종) 또는 **서울 자치구**를 고르면 감별 O/X·진짜 찾기가 그 범위 단지만 낸다(`lib/data.ts: poolOf`, `?area=`). 서울만 구 단위를 여는 이유는 서울만 전수 수집이 끝났기 때문. 가짜도 같은 범위 말투만 쓴다 — 진짜만 그 동네 이름이면 공짜 힌트가 된다. 지역 토큰은 `시도|시군구` 복합키로 잡는다(서울 중구와 부산 중구가 같은 키가 되면 서울 이름이 부산에 샌다). 이름 조립은 구역을 따르지 않고(퍼즐 수 부족), 구역끼리 순위를 비교하는 화면은 만들지 않는다.
