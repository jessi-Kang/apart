...
- 공식전 진입: 홈 접수 대장의 **공식전 열이 유일한 출전구**다(`/play?official=1` 등). 아직 안 치른 창구는 "출전하기 ›"(권유), 치른 창구는 인주색 "성적표" 도장(`.chop.done` — 변위 필터를 먹인 이중 테두리)이다. 도장은 이미 찍힌 것에만 쓴다. 무한 화면에서 공식전으로 건너가던 칩은 없앴다 — 대장이 제 몫을 하게 된 뒤로는 같은 입구가 둘이 되고, 무한을 치는 중에 공식전 회차 번호가 떠서 지금 뭘 치고 있는지 헷갈렸다.
