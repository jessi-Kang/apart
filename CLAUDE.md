...
- **문구를 추가·수정하면 `npm run check-layout`으로 레이아웃 회귀를 확인한다** (빌드 후 `npm start`가 떠 있어야 한다).
  마지막 줄에 두세 글자만 남는 고아 줄과, 상자 안에서 글자가 위아래로 쏠리는 결함을 화면별로 잡는다.
  같은 결함을 발견할 때마다 그 클래스만 고치는 방식은 새 문구를 넣을 때마다 되살아난다 — 그래서
  `globals.css`의 `body { text-wrap: pretty }`로 기본을 막고, 새는 것을 이 검사로 잡는다.
  전역 설치된 playwright를 빌려 쓰므로 없으면 검사를 건너뛴다(의존성은 늘리지 않는다).
