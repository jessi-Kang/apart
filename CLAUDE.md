...
- **UI나 문구를 고치면 `/qa` 스킬(`.claude/skills/qa`)을 돌린다.** 사람이 반복해서 눈으로
  잡아 준 결함 21가지를 레이아웃·동작·일관성·데이터·배포로 나눠 훑는다. 레이아웃만 볼 때는
...
- 공식전 진입: 홈 접수 대장의 **공식전 열이 유일한 출전구**다(`/o?official=1` 등). 아직 안 치른 창구는 "출전하기 ›"(권유), 치른 창구는 인주색 "성적표" 도장(`.chop.done` — 변위 필터를 먹인 이중 테두리)이다. 도장은 이미 찍힌 것에만 쓴다. 무한 화면에서 공식전으로 건너가던 칩은 없앴다 — 대장이 제 몫을 하게 된 뒤로는 같은 입구가 둘이 되고, 무한을 치는 중에 공식전 회차 번호가 떠서 지금 뭘 치고 있는지 헷갈렸다.
...
- 집계 DB: Neon Postgres (프로젝트 frosty-term-36707081, DB `aptgam`, 테이블 question_stats·score_dist·endless_runs·app_user·user_state). **로컬에서 쓰기 경로를 테스트할 때는 `DATABASE_URL=`을 비워서 띄운다** — `.env.local`에 운영 연결 문자열이 있어서 그냥 띄우면 테스트 기록이 운영 집계에 섞인다(실제로 한 번 섞여 지웠다). 연결 문자열은 `web/.env.local`(로컬)과 Vercel 배포의 env로만 주입하고 절대 커밋하지 않는다.
