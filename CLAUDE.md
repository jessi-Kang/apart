...
- `web/data/apartments.json`은 K-apt 실데이터 5,469건(서울 전역 2,889 + 경기·인천·부산·대구·대전·울산·세종 각 140~420건, 목록 V4 + 기본정보 V5로 수집·정제). 서울 외 지역은 기본정보 일일 쿼터(5,000) 때문에 시도당 430건씩 표본으로 받아 둔 상태이고, 이어서 실행하면 기존 수집본에 누적된다. **광주(시도코드 29)는 목록 API가 계속 0건을 돌려줘 아직 없다.** 갱신은 `npm run collect-kapt`(인증키는 `web/.env.local`의 `KAPT_API_KEY`, 절대 커밋 금지) → `npm run validate-pool` → `apartments.collected.json` 검토 후 승격. `fake_names.json`은 LLM 배치 생성 + validate-pool 대조를 거친 274건(화려한 조합형과 4~6자 단순형이 섞여 있어야 한다 — 한쪽으로 치우치면 길이가 곧 정답 힌트가 된다) — 추가 생성 시에도 같은 절차(생성 → validate-pool → 통과분만 등록)를 지킨다.
