...
import PIECES from "@/data/name_pieces.json";
 ...
 *
 * 어휘 자체는 data/name_pieces.json 한 곳에 있다. 가짜 생성기
 * (scripts/generate-fakes.mjs)도 같은 파일을 본다 — 같은 어휘를 두 곳에
 * 적으면 반드시 갈라지고, 그러면 작명소에서 지을 수 있는 이름과 생성기가
 * 짓는 이름의 결이 서로 달라진다.
 */
const BRANDS: string[] = PIECES.brands;
const OLD_MAKERS: string[] = PIECES.oldMakers;
const PLACE_WORDS: string[] = PIECES.place;
const GRADE_WORDS: string[] = PIECES.grade;
const KOREAN_WORDS: string[] = PIECES.korean;
